...
docker run --name colmap_container --security-opt seccomp=unconfined --add-host=host.docker.internal:host-gateway -td --gpus all -w /working -v $1:/working -it colmap/colmap:latest
...
docker exec colmap_container colmap -h
